...
(function() {
  jQuery(function($) {
    return $('#redemption_form').submit(function() {
      return $('#redeem_target').show();
    });
  });
...
(function() {
  jQuery(function($) {
    var Scoreboard, calculateWidth, extractLogo, logoHtml;
    if ($('#team_row_template').length !== 1) {
      return;
    }
    calculateWidth = function(score) {
      return score;
    };
    logoHtml = $($('#team_logos').html());
    extractLogo = function(id) {
      return logoHtml.filter("#logo_" + id).html();
    };
    Scoreboard = (function() {
      function Scoreboard() {
        this.body = $('tbody#scoreboard_body');
        this.path = this.body.data('refresh-path');
        this.interval = this.body.data('refresh-interval');
        this.template = $('#team_row_template').html();
        this.startPoll();
      }

      Scoreboard.prototype.startPoll = function() {
        return window.setTimeout(this.poll(), this.interval);
      };

      Scoreboard.prototype.poll = function() {
        return (function(_this) {
          return function() {
            return $.ajax({
              ...
              dataType: 'json',
              method: 'get',
              success: _this.receive()
            });
          };
        })(this);
      };

      Scoreboard.prototype.receive = function() {
        return (function(_this) {
          return function(data, textStatus, jqx) {
            var row, rows;
            rows = (function() {
              ...
              for (i = 0, len = ref.length; i < len; i++) {
                ...
                results.push(Mustache.render(this.template, row));
              }
              return results;
            }).call(_this);
            _this.body.html(rows.join());
            _this.body.attr('data-response-id', jqx.getResponseHeader('X-Request-Id'));
            return _this.startPoll();
          };
        })(this);
      };
    ...
    return Scoreboard.scoreboard = new Scoreboard;
  });
...
(function() {
  jQuery(function($) {
    ...
    Countdown = (function() {
      function Countdown() {
        this.timers = $('#timers');
        this.timerPath = this.timers.data('timer-path');
        this.interval = this.timers.data('timer-interval');
        this.template = this.timers.html();
        this.timers.html('');
        this.startPoll();
      }

      Countdown.prototype.startPoll = function() {
        return window.setTimeout(this.poll(), 0);
      };

      Countdown.prototype.requeue = function() {
        var finish, lag;
        finish = new XDate();
        lag = this.start.diffMilliseconds(finish);
        return window.setTimeout(this.poll(), this.interval - lag);
      };

      Countdown.prototype.poll = function() {
        return (function(_this) {
          return function() {
            _this.start = new XDate();
            if ((_this.recentTimers == null) || (_this.start > _this.recentTimers.next)) {
              return $.ajax({
                url: _this.timerPath,
                dataType: 'json',
                method: 'get',
              ...
              return _this.lazyUpdate();
            }
          };
        })(this);
      };

      Countdown.prototype.receive = function() {
        return (function(_this) {
          return function(data, textStatus, jqx) {
            var server;
            server = new XDate(data['time']);
            _this.adjustment = _this.start.diffMilliseconds(server);
            _this.recentTimers = data.timers;
            _this.recentTimers.next = _this.start.clone().addSeconds(15);
            return _this.lazyUpdate();
          };
        })(this);
      };

      Countdown.prototype.lazyUpdate = function() {
        var game, h, round, server, today;
        server = this.start.addMilliseconds(this.adjustment);
        game = this.toRemnant(server, new XDate(this.recentTimers.game));
        today = this.toRemnant(server, new XDate(this.recentTimers.today));
        round = this.toRemnant(server, new XDate(this.recentTimers.round));
        h = Mustache.render(this.template, {
          game: game,
          today: today,
          round: round
        });
      ...
      Countdown.prototype.toRemnant = function(now, ending) {
        var diff;
        try {
          diff = {
    ...
    return Countdown.countdown = new Countdown;
  });
